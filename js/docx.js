...
function convertContent(input) { 'use strict'; // Convert HTML to WordprocessingML, and vice versa
    ...
    if (input.files) { // input is file object
        ...
        var mainDocument = input.files['word/document.xml'].async("string").then(function (data) {
            var output, inputDoc, i, j, k, id, doc, inNode, inNodeChild, outNode, outNodeChild, styleAttrNode, pCount = 0, tempStr, tempNode, val, footnoteId = {value : 1};
        ...
        var endnotes = input.files['word/endnotes.xml'].async("string").then(function (data) {
        ...
        return Promise.all([styles, mainDocument, endnotes]).then(function (results) {
            var output = {
               styles: results[0],
